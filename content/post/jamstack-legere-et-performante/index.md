...
Un premier épisode assez dense, pour cause d'actualité très fournie, et la volonté d’être aussi didactiques que possible. Dans cet épisose Nicolas Goutay explique, en quoi l’adoption d’un générateur qui n’embarque pas de framework JavaScript par défaut, va avoir un impact significatif sur la performance ressentie par une majorité de personnes qui utilisent un téléphone d’entrée de gamme.
...
Près de 80 articles ont été publiés depuis, plusieurs meetups ont eu lieu notamment sur Paris. Au vu des nombreuses discussions sur le Slack, nous avons décidé de nous essayer au format podcast, dans l’espoir de partager toujours plus sur les sites web modernes et performants.
...
Des frameworks bien pratiques, de plus en plus versatiles, capables de travailler avec des fichiers locaux comme avec des APis distantes. Ces frameworks JavaScript facilitent le développement par composition, ce paradgime explique en partie leur adoption de plus en plus massive.
...
[Tailwind CSS dont la version 2.0 est sortie](https://blog.tailwindcss.com/tailwindcss-v2).[Le site fait peau neuve](https://tailwindcss.com) pour l'occasion, il est développé avec Next.js, déployé sur Vercel avec une recherche propulsée par Algolia. Ce genre de stack s'impose comme une évidence pour les landing pages et les sites de documentation.
...
Les langages de templating proposés par les générateurs sont néanmoins moins versatiles que React ou Vue. On utilsera des fichiers partiels pour tendre vers un développement par composition, mais c'est une expérience de développement différente.
