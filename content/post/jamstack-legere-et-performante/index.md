---
title: "Nicolas Goutay, vers une Jamstack légère et performante"
description: "Passer de Gatsby à Eleventy afin de pouvoir ciseler un site web plus performant et accessible à tous."
date: 2020-11-19
draft: true
images:
  - https://res.cloudinary.com/jamstatic/image/upload/f_auto,q_auto/w_1200,c_fit,co_white,g_north_west,x_80,y_80,l_text:poppins_80_ultrabold_line_spacing_-30:Nicolas%2520Goutay%2520%253A%2520une%2520Jamstack%2520plus%2520l%25C3%25A9g%25C3%25A9re%2520et%2520performance/jamstatic/twitter-card.png
categories:
  - podcast
  - gatsby
  - eleventy
...
Un premier épisode assez dense, car outre une actualité très fournie, nous avons essayé d’être aussi didactiques que possibles sur certains aspects. Une première partie liée aux dernières annonces des différents frameworks JS, dans la deuxième partie Nicolas Goutay explique en détail, en quoi l’adoption d’un générateur qui n’embarque pas de framework JS par défaut, va avoir un impact significatif sur la performance ressentie par une majorité de personnes qui utilisent un téléphone d’entrée de gamme.

## Intro

Jamstatic a été lancé avec la volonté de partager autour des site statiques, nous suivons avec attention cette actualité depuis près de 5 ans. Près de 80 articles ont été publiés depuis, plusieurs meetups ont eu lieu notamment sur Paris. Au vu des nombreuses discussions sur le Slack, nous avons décidé de nous essayer au format podcast, dans l’espoir de partager toujours plus sur les sites web modernes et performants.
...
Gatsby.js, un des deux frameworks React majeurs qui permet de générer des applications web, a d’ailleurs [annoncé avoir finalement opté pour WordPress en mode headless pour son blog](https://www.gatsbyjs.com/blog/gatsby-blog-wordpress). Ils permettent ainsi à plus de 130 contributeurs de rester dans leur zone de confort, tout en contournant les tarifs de CMS leaders sur le marché qui dans sa formule actuelle à $489 dollars n’intègre que 25 utilisateurs. Une stratégie donc moins frontale et plus intelligente de Gatsby qui par la même occasion montre qu’on peut très bien avoir un chouette site React en front en gardant son WordPress en back en requétant les données en GraphQL.

L’autre framework React qui a le vent en poupe c’est [Next.js](https://nextjs.org/). La première [conférence](https://nextjs.org/conf/schedule) du framewrok hybride qui permet de faire du statique *et* du SSR a eu lieu fin octobre. 

Beaucoup d’annonces de la part de Vercel, notamment la mise à disposition d’un outil de monitoring de métriques de performance ([Core Web Vitals](https://web.dev/vitals/#core-web-vitals)) mesurées depuis leur CDN auprès des utilisateurs sans avoir à ajouter la moindre ligne de JS à son site. 

Ces [métriques](https://web.dev/metrics/) poussées par Google permettent de démontrer la performance de sites développés avec Next.js et déployés sur les CDN de Vercel. Ils ont d’ailleurs aussi à l’occasion sorti un premier exemple de site e-commerce et annonce bientôt supporter de s'interfacer avec l'API de Shopify. Auand on connaît l’importance de la performance sur les taux de conversions en e-commerce, on se dit que la bataille va faire rage dans ce secteur. On notera  que la plate-forme de déploiement Gatsby Cloud permet aussi de mesurer son score Lighthouse standard, des indicateurs "un peu fourre-tout", selon Nicolas. Il existe beaucoup de métriques en webperf, celles choisies par Vercel sont le temps mis pour commencer à afficher quelque chose, 
...
Le projet initié par les deux frères Chopin, Alexandre et Sébastien, a annoncée sa première levée de fonds et propule maintenant des sites majeurs comme lequipe.fr. Super boulot.
...
Des frameworks bien pratiques, de plus en plus versatiles, capables de travailler avec des fichiers locaux comme avec des APis distantes. Ces frameworks JavaScript facilitent le développement par composition, cette flexibilité explique sans doute en partie leur adoption de plus en plus massive.
...
De son côté, mine de rien [Eleventy le générateur volontairement simple]({{< relref "eleventy-generateur-statique-simple" >}}) de Zach Leatherman se rapproche doucement du million de téléchargements npm, l’engouement est réel chez les développeurs front adeptes des choses bien faites, et souhaitant garder un contrôle total sur le JavaScript livré. C'est d'ailleurs ce qui a plu à Nicolas Goutay, qui détaille [l'approche plus légére privilégiée pour développer le nouveau site d'Orbit](https://orbit.love/blog/towards-a-lightweight-jamstack), avec Eleventy, TailwinCSS et Alpine.js.

[TailwindCSS dont la version 2.0 est sortie](https://blog.tailwindcss.com/tailwindcss-v2).[le site fait peau neuve](https://tailwindcss.com) pour l'occasio,, il est développé avec Next.js, déployé sur Vercel avec une recherche propulsée par Algolia. 

Si vous devez servir une même version d'un site de contenu à tous vos visiteurs, en y ajoutant un peu d'interactivité, le servir en statique est le meilleure moyen de proposer quelque chose de simple et performant.
...
Nicolas a longtemps utiliser React pour beaucoup de projets en agence. Très attentif à la performance finale ressentie par les visiteurs, Nicolas sait que cela demande d'implémenter beaucoup de bonnes pratiques: la gestion des images, l'implémentation de Service Workers, le rendu côté serveur. "Encore faut-il en avoir le temps, l'expertise et la connaissance". Gatsby s'est positionné comme un framework React qui se connecter à n'importe quelle source de données via son API GraphQL et qui implémente les bonnes pratiques de webperf par défaut, en générant une Progressive Web App en sortie. 

Gatsby peut aussi précharger les pages dès qu'un visiteur s'apprête à cliquer sur un lien, ce qui va donner une impression d'instantanéité du chargement de la page. C'est ce parti-pris sur la performance qui a plu à Nicolas, qui fait que le développeur est incité à servir un site rapide.

Nicolas rappelle néamoins que la taille du JavaScript livré sur le site a un impact énorme sur l'expérience utilisateur (et le SEO dans une bien moindre mesure). Tout le monde ne maintient pas une application comme Facebook, Airbnb, ou Verce et même 30 kilos de JavaScript vont impacter le temps que met un site à être interactif. Cela peut se mesurer en terme de clics rageux où les utilisateurs croient pouvoir interagir avec la page qui semble chargée, alors que le navigateur est encore en train d'interpréter le code JavaScript embarqué.
...
On surveillera le [First Input Delay](https://web.dev/fid/) dans ses métriques, même si comme le dit Nicolas "cela reste une approximation synthétique et imparfaite". Idéalement testez sur autre chose que le dernier Iphone avec une connexion 4G.
...
Les langages de templating proposés par les générateurs sont néanmoins moins versatiles que React ou Vue. On utilsera des fichiers partiels pour simuler un développement par composition, mais c'est une expérience de développement différente.
...
Certains sites Eleventy utilisent les Web Components. GitHub a également développer [un framework pour embarquer des Web Components dans les applications Ruby on Rails](https://github.com/github/view_component).

Il est toujours possible d'utiliser Vue avec Eleventy, c'est d'ailleurs [ce qu'a fait Zach Leatherman sur netlify.com](https://www.netlify.com/blog/2020/09/18/eleventy-and-vue-a-match-made-to-power-netlify.com/).

Le statique moderne est toujours en plein évolution, beaucoup d'outils donc pour produire des sites à la fois modernes, accessibles et performants.
