...
Dans ce post, nous allons voir certains cas d'usage d'une appli web complexe, comme le fait d'aller requêter des données tierces dynamiques ou l'authentification, and nous allons révéler pourquoi Gatsby est un très bon choix pour construire des applis web.
...
Pour illustrer ce concept, commençons avec une exemple classique… Nous avons besoin de requêter des points de donnée au runtime plutôt qu'au build-time.
